...
class DharmaNitiStrategy:
    ...
    def decide_move(self) -> str:
        ...
        # Compute features safely (added: prevents early-round errors)
        ...
        if self.retaliation_remaining > 0:
            ...
            # Added: auto-exit retaliation phase cleanly
            if self.retaliation_remaining == 0:
                self.in_retaliation_phase = False
        ...
        if self._should_forgive(features):
            # Added: full reset prevents aggression leakage
            self._reset_retaliation_state()
            return "C"

        #Rule 4: Respond to opponent's last move
        # Added: defensive check to avoid unsafe index access
    ...
    def _compute_features(self) -> dict:
        ...
        # Added: bounded recent window safety
        recent_len = min(self.RECENT_WINDOW, total)
        recent_slice = self.opponent_history[-recent_len:]
        recent_coop = recent_slice.count("C") / recent_len

        own_coop = self.own_history.count("C")

        # Added: betrayal rate meaningful only after enough cooperation
        betrayal_rate = (
    ...
    def _should_forgive(self, features: dict) -> bool:
        ...
        # Added: ensure forgiveness window is complete
        return (
            len(recent_moves) == self.FORGIVENESS_WINDOW
            and all(move == "C" for move in recent_moves)
    ...
    def _start_retaliation(self, length: int):
        # Added: prevent retaliation stacking and infinite escalation
        if not self.in_retaliation_phase and length > 0:
            self.retaliation_remaining = max(1, min(3, length))
            self.in_retaliation_phase = True

    def _reset_retaliation_state(self):
        # Added: full emotional reset after forgiveness
        self.retaliation_remaining = 0
        self.in_retaliation_phase = False
        self.consecutive_opponent_defections = 0
